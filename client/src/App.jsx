...
// 1. SETUP API CONNECTION (Talks to Render for Emails)
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';

// 2. SETUP AUTH (Supabase for Login only)
...
function Dashboard() {
  ...
  const safeTrucks = vehicles.filter(v => v.status !== 'VOR');
    ...
    return (
      <div className="card-hover" style={{ backgroundColor: 'white', padding: '15px', borderRadius: '8px', marginBottom: '12px', border: '1px solid #e2e8f0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
          <div><h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '700', color: '#0f172a' }}>{truck.reg_number}</h3><p style={{ margin: '2px 0 0 0', fontSize: '0.8rem', color: '#64748b' }}>{truck.make}</p></div>
    ...
    <div style={{ fontFamily: '"Inter", sans-serif', backgroundColor: '#f8fafc', minHeight: '100vh', width: '100%', margin: 0, padding: 0 }}>
      <style>{`
        ...
        @media (min-width: 1024px) {
            ...
            .board-container { grid-template-columns: 1fr 1fr; padding: 30px; gap: 30px; height: calc(100vh - 160px); }
        ...
        <div className="board-container">
          <div className="column" style={{ background: '#f8fafc' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', borderBottom: '2px solid #22c55e', paddingBottom: '10px' }}>
              <span style={{ fontWeight: '800', color: '#166534', fontSize: '0.9rem' }}>ACTIVE FLEET</span>
          ...
          <div className="column" style={{ background: '#f8fafc' }}>
             <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', borderBottom: '2px solid #ef4444', paddingBottom: '10px' }}>
              <span style={{ fontWeight: '800', color: '#991b1b', fontSize: '0.9rem' }}>VOR / ISSUES</span>
